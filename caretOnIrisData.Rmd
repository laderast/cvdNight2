...
A full list of machine learners is available here: http://topepo.github.io/caret/available-models.html
...
#train randomForest
rfIris <- train(Species ~ Sepal.Width + Petal.Width, method= "ranger", data=trainData)
```
...
classPredRF <- predict(rfIris, newdata = testData)

#compare all the predictions directly
#were there any rows where the predictions didn't match?
data.frame(truth=testData$Species, classPredLDA, classPredCart, classPredLDA)
```
...
confMatRF <- confusionMatrix(classPredRF, testData$Species)

accuracyComparison = rbind(LDA = confMatLDA$overall,
